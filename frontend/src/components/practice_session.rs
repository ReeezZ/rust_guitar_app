...
pub fn PracticeSession(
  ...
  let handle_root_note_update = {
    ...
    Callback::new(move |new_note: Note| {
      if let (Some(ref ex), Some(ref callback)) = (&ex_ref, &callback) {
        let updated_exercise = match &ex.exercise_type {
          ExerciseType::Scale {
            scale_type,
            fret_range,
            ..
          ...
          ExerciseType::Triad {
            ...
            name: ex.name.clone(),
            description: ex.description.clone(),
            exercise_type: ExerciseType::Triad {
              root_note: new_note,
              scale_type: *scale_type,
              fret_range: *fret_range,
            },
                                ...
                                <button
                                  ...
                                  on:click=move |_| {
                                    if let Some(selected_note) = temp_selected_note.get() {
                                      ...
                                      set_show_root_note_modal.set(false);
                                      handle_root_note_update.run(selected_note);
                                    }
                                  }
                                >
                                  "OK"
