...
fn ConfigurationSection(
  ...
  let update_exercise_with_changes = Callback::new(move |_| {
    ...
    updated_exercise.exercise_type = match updated_exercise.exercise_type {
      ExerciseType::Scale { .. } => ExerciseType::Scale {
      ...
      ExerciseType::Triad { .. } => ExerciseType::Triad {
        root_note: selected_root_note.get(),
        scale_type: selected_scale_type.get(),
        fret_range: (selected_start_fret.get(), selected_end_fret.get()),
      },
  ...
  view! {
    <div class="p-4 mb-6 bg-gray-50 rounded-lg">
      <h3 class="mb-3 text-lg font-semibold text-gray-700">"Exercise Configuration"</h3>
      <div class="flex flex-wrap gap-3">

        // Root Note Button (Green)
        <div class="relative">
          <button
            class="py-2 px-4 font-medium text-white bg-green-500 rounded-lg shadow-sm transition-colors cursor-pointer hover:bg-green-600"
            on:click=move |_| {
          ...
          >
            "Root: "
            {move || selected_root_note.get().to_string()}
            <svg
          ...
          <Show when=move || show_root_note_dropdown.get()>
            <div class="absolute left-0 top-full z-10 mt-1 bg-white rounded-lg border border-gray-300 shadow-lg min-w-[150px]">
              {note_options
                .iter()
                .map(|&note| {
                  view! {
        ...
        <div class="relative">
          <button
            class="py-2 px-4 font-medium text-white bg-purple-500 rounded-lg shadow-sm transition-colors cursor-pointer hover:bg-purple-600"
            on:click=move |_| {
          ...
          >
            "Scale: "
            {move || match selected_scale_type.get() {
              ScaleType::Hepatonic(HeptaScaleType::Major) => "Major",
              ScaleType::Hepatonic(HeptaScaleType::Minor) => "Minor",
              _ => "Unknown",
            }}
        ...
        <div class="relative">
          <button
            class="py-2 px-4 font-medium text-white bg-orange-500 rounded-lg shadow-sm transition-colors cursor-pointer hover:bg-orange-600"
            on:click=move |_| {
          ...
          >
            "Frets: "
            {move || format!("{}-{}", selected_start_fret.get(), selected_end_fret.get())}
            <svg
              class="inline-block ml-1 w-4 h-4"
              fill="none"
              stroke="currentColor"
          ...
          <Show when=move || show_fret_range_dropdown.get()>
            <div class="absolute left-0 top-full z-10 p-4 mt-1 bg-white rounded-lg border border-gray-300 shadow-lg min-w-[200px]">
              <div class="space-y-3">
                <div>
                  <label class="block mb-1 text-sm font-medium text-gray-700">"Start Fret"</label>
                  <input
                    type="number"
                    min="0"
                    max="24"
                    class="py-2 px-3 w-full rounded border border-gray-300 focus:ring-2 focus:ring-orange-500 focus:outline-none"
                ...
                <div>
                  <label class="block mb-1 text-sm font-medium text-gray-700">"End Fret"</label>
                  <input
                    type="number"
                    min="0"
                    max="24"
                ...
                <div class="flex justify-end pt-2 space-x-2">
                  ...
                  <button
                    class="py-1 px-3 text-sm text-white bg-orange-500 rounded hover:bg-orange-600"
                    on:click=move |_| {
                      set_show_fret_range_dropdown.set(false);
                      update_exercise_with_changes.run(());
                    }
