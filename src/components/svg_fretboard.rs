...
pub fn SvgFretboard(num_frets: Signal<usize>) -> impl IntoView {
    let svg_width: f64 = 900.0;
    let svg_height: f64 = 320.0;
    let num_strings = 6;
    ...
    view! {
        ...
        >
            {move || {
                ...
                let fret_positions = calculate_fret_positions(svg_width, num_frets as u8);
                ...
                // Simple visual cue for guitar body at the end of the fretboard
                let body = {
                    let last_fret_x = fret_positions[num_frets as usize];
                    let body_x = last_fret_x + 16.0; // leave some space after last fret
                    let body_width = svg_width - body_x;
                    view! {
                        <rect
                            x=body_x
                            y=fret_margin
